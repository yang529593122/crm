				...
				<el-table :data="tableData.records"  style="width: 100%" border>
			        ...
			        <el-table-column label="操作" prop="operation"  align="center" >
			          <template slot-scope="scope"   >
			            <el-button type="primary" size="mini" @click="editRemark(scope.$index, scope.row)" v-show="twoStatus" >编辑</el-button>
			            <el-button type="primary" size="mini" @click="delExamine(scope.$index, scope.row)" v-show="oneStatus" >删除</el-button>
			          </template>
			        </el-table-column>
			      </el-table>
			</div>
...
<script>
	export default{
		...
		computed:{
			...
			oneStatus(){
				if(this.$store.state.examineData.status==0){
					return true
				}else{
					return false
				}
			...
			twoStatus(){
				if(this.$store.state.examineData.status==1){
					return true
				}else{
					return false
				}
		...
		methods:{
			...
			delExamine(index,row){							
				...
				this.$http.post("task/dtl/delete",obj,{
			  		headers:{
			  			"Content-Type":"application/json",				  						  			
			  		},
			  		transformRequest: [(data, headers)=> {let keys = Object.keys(headers);let normalizedName = 'Content-Type';
